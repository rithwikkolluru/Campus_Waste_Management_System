...
exports.getAllReports = async (req, res) => {
  ...
  let queryText = `
    SELECT r.*, u.name as reporter_name, z.name as zone_name
    FROM reports r
  ...
  if (zone) {
    params.push(zone);
    queryText += ` AND z.name = $${params.length}`;
  }
  if (priority) {
  ...
  try {
    const result = await db.query(
      `INSERT INTO reports (user_id, zone_id, description, waste_type, priority, image_url, status)
       ...
       RETURNING *`,
      [user_id, zone_id, description, waste_type, priority, image_url]
    );
...
exports.updateStatus = async (req, res) => {
  ...
  try {
    ...
    // 3. Log the status change (Table 7)
    await db.query(
